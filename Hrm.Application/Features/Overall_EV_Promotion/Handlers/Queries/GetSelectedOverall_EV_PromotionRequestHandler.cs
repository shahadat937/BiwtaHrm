...
{ 
    public class GetSelectedOverall_EV_PromotionRequestHandler : IRequestHandler<GetSelectedOverall_EV_PromotionRequest, List<SelectedModel>>
    {
        ...
        public GetSelectedOverall_EV_PromotionRequestHandler(IHrmRepository<Hrm.Domain.Overall_EV_Promotion> Overall_EV_PromotionRepository)
        {
        ...
        {
            ICollection<Hrm.Domain.Overall_EV_Promotion> Overall_EV_Promotions = await _Overall_EV_PromotionRepository.FilterAsync(x => x.IsActive);
            List<SelectedModel> selectModels = Overall_EV_Promotions.Select(x => new SelectedModel 
            {
                Name = x.Overall_EV_PromotionName,
                Id = x.Overall_EV_PromotionId
            }).ToList();
            return selectModels;
        }
    }

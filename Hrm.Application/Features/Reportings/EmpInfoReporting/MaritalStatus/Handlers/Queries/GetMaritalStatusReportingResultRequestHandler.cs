...
{
    public class GetMaritalStatusReportingResultRequestHandler : IRequestHandler<GetMaritalStatusReportingResultRequest, PagedResult<EmpReportingSearchResultDto>>
    {

        private readonly IHrmRepository<EmpBasicInfo> _EmpBasicInfoRepository;
        public GetMaritalStatusReportingResultRequestHandler(IHrmRepository<EmpBasicInfo> EmpBasicInfoRepository)
        {
            _EmpBasicInfoRepository = EmpBasicInfoRepository;
        }

        public async Task<PagedResult<EmpReportingSearchResultDto>> Handle(GetMaritalStatusReportingResultRequest request, CancellationToken cancellationToken)
        {
            if (request.UnAssigned == false)
            {
                IQueryable<EmpBasicInfo> query = _EmpBasicInfoRepository.FilterWithInclude(x =>
                    (request.Id == 0 || x.EmpPersonalInfo.FirstOrDefault().MaritalStatusId == request.Id) &&
                ...
                var resultData = await query
                    .Select(x => new EmpReportingSearchResultDto
                    {
                        ...
                        TypeName = x.EmpPersonalInfo.FirstOrDefault().MaritalStatus.MaritalStatusName ?? "",
            ...
            {
                IQueryable<EmpBasicInfo> query = _EmpBasicInfoRepository.FilterWithInclude(x =>
                (x.EmpPersonalInfo.FirstOrDefault().MaritalStatusId == null) &&
                (request.DepartmentId == 0 || x.EmpJobDetail.FirstOrDefault().DepartmentId == request.DepartmentId) &&
                (request.SectionId == 0 || x.EmpJobDetail.FirstOrDefault().SectionId == request.SectionId))
                .Include(x => x.EmpJobDetail)
                    .ThenInclude(x => x.Department)

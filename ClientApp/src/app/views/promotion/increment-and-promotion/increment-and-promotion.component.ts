...
export class IncrementAndPromotionComponent implements OnInit, OnDestroy {
    ...
    this.subscription.push(
      this.empPromotionIncrementService.findById(this.id).subscribe((res) => {
        if (res) {
          ...
          res.effectiveDate = this.sharedService.parseDate(res.effectiveDate)
          res.approveDate = this.sharedService.parseDate(res.approveDate)
  ...
  onSubmit(form: NgForm): void {
    ...
    const formatedCurrentDeptJoinDate = this.sharedService.formatDateOnly(this.empPromotionIncrementService.empPromotionIncrement.currentDeptJoinDate)
    const formatedOrderDate = this.sharedService.formatDateOnly(this.empPromotionIncrementService.empPromotionIncrement.orderDate)
    const formatedEffectiveDate = this.sharedService.formatDateOnly(this.empPromotionIncrementService.empPromotionIncrement.effectiveDate)
    const formatedApproveDate = this.sharedService.formatDateOnly(this.empPromotionIncrementService.empPromotionIncrement.approveDate)

    const payload = {
      ...form.value,
      currentDeptJoinDate: formatedCurrentDeptJoinDate,
      orderDate: formatedOrderDate,
      effectiveDate: formatedEffectiveDate,
      approveDate: formatedApproveDate
    ...
    const action$ = id
      ? this.empPromotionIncrementService.updateEmpPromotionIncrement(id, payload)
      : this.empPromotionIncrementService.saveEmpPromotionIncrement(payload);
    // const action$ = id

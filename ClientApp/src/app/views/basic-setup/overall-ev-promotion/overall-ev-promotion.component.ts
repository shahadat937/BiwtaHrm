...
export class OverallEVPromotionComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  handleRouteParams() {
    this.route.paramMap.subscribe((params) => {
      const id = params.get('overall_EV_PromotionId');
      if (id) {
        this.btnText = 'Update';
  ...
  initaialOverall_EV_Promotion(form?: NgForm) {
    ...
    this.overall_EV_PromotionService.overall_EV_Promotions = {
      overall_EV_PromotionId: 0,
      overall_EV_PromotionName: '',
      menuPosition: 0,
      isActive: true,
    };
  }
  ...
  delete(element: any) {
    this.confirmService
      ...
      .subscribe((result) => {
        if (result) {
          this.overall_EV_PromotionService.delete(element.overall_EV_PromotionId).subscribe(
            (res) => {
